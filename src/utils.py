...
def to_mongodb(pred_input):
    try:
        uri = "mongodb+srv://testdelivery:prajwal@delivery.idegko8.mongodb.net/?retryWrites=true&w=majority"
        ...
        logging.info("Error Occured at databse stage in mongoDB")
        raise CustomException(e,sys) '''
...
def to_mongodb(pred_input):
    try:
        ...
        print(mongodb_password,mongodb_username)
        uri = f"mongodb+srv://{mongodb_username}:{mongodb_password}@delivery.idegko8.mongodb.net/?retryWrites=true&w=majority"
        # Replace 'your_connection_string' with your actual MongoDB Atlas connection string
        #client = MongoClient(uri)
        client = MongoClient(uri, server_api=ServerApi('1'))
